...
float *list_Slv_flt[]={
&Slv_float[0],      //1000 0 операнд команды
&Slv_float[1],      //1002 1 копия операнда
              // резервные переменные
&Vol_beg,           // 1004 2  объем цистерны до уровня погружения рукава,л
&P_min_valve,       // 1006 3  минимальное давление  МПа, при котором возможно увеличить степень
                    //    открытия вентиля
&T_Ref,             // 1008 4  Референсная температура,C (15)
&D_ref,             // 1010 5  Плотность приведенная к референсной температуре
&V_ref,             // 1012 6  Объем отпуска приведенный к референсной температуре
&Temp_a,            // 1014 7  Температура отпущенного нефтепродукта
&Dens_a,            // 1016 8  Плотность отпущенного нефтепродукта
&Slv_float[9],      // 1018 9
              // Данные из расходомера
&s_MVD[0].MassI,    // 1020 10  Масса инвентарная ( накапливающий счетчик) Mass inventory ,I263
&s_MVD[0].MassT,    // 1022 11  Масса в текущей операции отпуска (total)   Mass total     ,I259
&s_MVD[0].FlowM,    // 1024 12  Массовый расход, кг/ч                      Mass flow      ,I247
&s_MVD[0].Dens,     // 1026 13  Плотность кг/м3                            Density        ,I249
&s_MVD[0].Temp,     // 1028 14  Температура,                               Temperature    ,I251
&s_MVD[0].VolI,     // 1030 15  Объем инвентарный ( накапливающий счетчик) Volume invntщry,I265

&s_MVD[0].VolT,     // 1032 16  Объем в текущей операции отпуска (total)
&s_MVD[0].FlowV,    // 1034 17  Объемный расход, л/ч
&s_MVD[0].PressG,   // 1036 18

                    // Данные аналоговых датчиков
&s_MVD[0].TempR,    // 1038 19  температура по внешнему датчику
&s_MVD[0].Press,    // 1040 20  давление по внешнему датчику
...
long int *list_Slv_lng[]={
                    // Addr = 1042
&P_dem,             //  1042 21  Заданное давление  10000 = 1 МПа
&LIM_P,             //  1044 22  частота насоса max,12800 = 1 Гц
&LIM_M,             //  1046 23  частота насоса min,12800 = 1 Гц
&Flow_beg,          //  1048 24  Расход начальный, л/с
&Flow_nom,          //  1050 25  Расход номинальный, л/с
&Flow_min,          //  1052 26  Расход замедления  , л/с
&Flow_min1,         //  1054 27  Расход замедления 1, л/с
&Flow_min2,         //  1056 28  Расход замедления 2, л/с
&Com_Baud_Slave,    //  1058 29  Частота обмена по Modbus
...
&Flow_beg      , T_INT_L         // 2   F1048  Расход начальный, л/с
};
