...
int Delta_seq[16]={0,3,4,0,3,4,0,3,4,0,3,4,0,3,4,0};
...
//                        адрес
unsigned char Dlt_Set_mode[6]={1,6,0x02,0x01,0x00,0x03};  // 3 - включение работы через управляющее слово
                                                         // RS485 + кн. STOP на панели
                                                         // 1 - External terminals + Stop bottom

//unsigned char Dlt_Set_mode[]={1,6,0x02,0x01,0x00,0x01};  // включение работы через внешние терминалы
                                                           //  кн. STOP на панели

unsigned char Dlt_Set_ctr[]={1,6,0x02,0x00,0x00,0x04};   // включение уставки скорости через RS485
...
unsigned char Dlt_Rd_curr[] ={1,3,0x21,0x04,0x00,0x01};   // управляющее слово
...
struct COM_rqst Delta_WD_Rqst[max_Dlt]={
//----------------------
// device 1
//--------------------------
0,                     //  состояние запроса
ffgets_com_rtu,        //  gets_com
f_Delta_WD_rd,         //  answ_com
f_Delta_WD_rd,         //  answ_flt
"Dlt_WD/1",             //  name
0,                     //  n_dev номер устройства в группе однотипных устройств
&Delta[0],             //  *ICP_dd
0,                     //  текущая функция
...
//
 ...
 int count;
  ...
  Delta[nn].n_success++;
  Delta_inp_err[nn]=0;
  f_queue_free(ii,&Delta_WD_Rqst[nn]);
  return;
...
// ----------------------------------------
long int Dlt_wd_time[max_Dlt]={0,0};
long int Dlt_pool_period[max_Dlt]={200,200};
int Dlt_wd[max_Dlt]={0,0};

void  f_Dlt_wd( int ii)
{
  // Watch Dog для Commander Dlt
  // ii - номер устройства
...
long int i1;
 ...
 if(FL_err) return;
 if( f_timer(Dlt_wd_time[ii],Dlt_pool_period[ii] ))
 {
  if((Delta_WD_Rqst[ii].status == Req_Flt) ||
     (Delta_WD_Rqst[ii].status == Req_OK)) ;
  else return;

           Dlt_wd[ii] ^= Dlt_WDT;

           itmp=Delta_Ctrl[ii] | Dlt_wd[ii];
           i1=6;
           buf_tmp[0]=Delta[ii].addr;
           buf_tmp[1]=6;
           ...
           i2=i1;
           f_prepareRTU(buf_tmp,&i2);
           _fmemcpy(Delta_WD_Rqst[ii].Cmd,buf_tmp,(long)i2);
           Delta_WD_Rqst[ii].cmd_lgth=i2;
           Delta_WD_Rqst[ii].time_stamp=TimeStamp;
           Dlt_wd_time[ii]=TimeStamp;
           Delta[ii].n_transaction++;
           f_queue_put(Delta[ii].port, &Delta_WD_Rqst[ii]) ;
 }
}
*/
// ----------------------------------------
