...
float CL_valV[3]={120,28,0};


int num_out1=1,num_out2=2;
int num_out3=4,num_out4=0;
int num_out5=5,num_out6=6;
int num_out7=3,num_out8=7;

int num_in1=1,num_in2=2;
int num_in3=3,num_in4=4;
int num_in5=5,num_in6=6;
int num_in7=7,num_in8=8;

// 25-30 л/мин = 500 мл/сек
//     5 л/мин =  83 мл/сек

int t_ctrl=0;   //  1 - изменять cutoff при отпуске
...
float CL_valV0=200;     // l, оставш. V в момент выключения
...
//printf("\r\n f_MVD_cor() ");
 ...
 D_ref=f_get_ref_Dns( Dens_AD, Temp_AD,T_Ref);
...
m_end:
    ...
    if ( Flag_mass != 0 )
    {
     ...
     if(fct_delay != 0)
     {
       CL_T0 += fct_delay*dsc_tmp;  // больший
       CL_T1 += fct_delay*dsc_tmp;  // средний
       CL_T2 += fct_delay*dsc_tmp;  // меньший
     }
    }
    else
    {
     if(fct_offs != 0)
     {
       CL_valV0 += fct_offs*offs_cor;    // больший
       CL_valV[0] += fct_offs*offs_cor;  // средний
       CL_valV[1] += fct_offs*offs_cor;
     }
     if(fct_delay != 0)
     {
       CL_T0 += fct_delay*dsc_tmp;  // больший
