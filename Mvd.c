...
#if(Shtray==2)
  -1.0, //float MassT_st       //float старший счетчик
  -1.0, //float VolT_st        //float старший счетчик
  -1.0, //float MassT_st       //float младший счетчик
  -1.0, //float VolT_st        //float младший счетчик
#endif
...
#if(Shtray==2) //Promass
  int MVD_seq[16]={
  //mSlot, mDens,mSlot, mSlot, //  mMassI,
  //mSlot, mTemp,mSlot, mSlot, //mVolI,
  //mSlot, mDens,mSlot, mSlot, //mMassI,
  //mSlot, mTemp,mSlot, mSlot, //mVolI,
...
#else //Shtray==0 - MicroMotion  or Shtray==1 - Shtray
  int MVD_seq[16]={
  mSlot, mSlot,mSlot, mSlot,
  mSlot, mSlot,mSlot, mSlot,
  mSlot, mSlot,mSlot, mSlot,
...
//08.02.2022 YN
#if(Shtray==2)
  unsigned char MVD_Rd_err[] ={1,3,0x0A,0xAB,0x00,0x01};  //2731 R2732 - регистр ошибки
  
  unsigned char MVD_Rd_FlowM[]={1,3,0x07,0xD6,0x00,0x02}; //2006  R2007  - mass flow
  unsigned char MVD_Rd_Dens[]={1,3,0x07,0xDC,0x00,0x02};  //2012  R2013  - Density
  unsigned char MVD_Rd_Temp[]={1,3,0x07,0xE0,0x00,0x02};  //2016  R2017  - Temperature
  unsigned char MVD_Rd_FlowV[]={1,3,0x07,0xD8,0x00,0x02}; //2008  R2009  - volume flow

  unsigned char MVD_Rd_Press[]={1,3,0x08,0x28,0x00,0x02}; //2088 R2089   - pressure

  unsigned char MVD_Rd_MassT[]={1,3,0x0A,0x31,0x00,0x02};  //2609  R2610  - mass total
  unsigned char MVD_Rd_VolT[] ={1,3,0x0A,0xF9,0x00,0x02};  //2809 R2810  - volume total
  unsigned char MVD_Rd_MassI[]={1,3,0x0A,0x31,0x00,0x02};  // R2610  - mass inventory
  unsigned char MVD_Rd_VolI[]= {1,3,0x0a,0xF9, 0x00,0x02}; // R2810  - Volume inventory
#else
  ...
  //08.02.2022 YN
  #if(Shtray==2)
    unsigned char MVD_Rd_Slot[]={1,3,0x13,0xBA,0x00,18};  // R5050    - First register (0245,Error) , 22 registers 16 bit
  #endif
...
struct mvd_init MVD_i[]=
{
 5001,2610, //сч1 мл Mass total-Inv  
 5002,2612, //сч1 ст Mass total-Inv     687 --
 5003,2810, //сч2 мл Vol total-Inv     687 --
 5004,2812, //сч2 ст Vol total-Inv     687 -- 
 5005,2013, // Density        689 --
 5006,2017, // Temperature    691 --
 5007,2007, // Mass flow      697 --
 5008,2009, // volume flow      697 --
 //5009,2732, // Error сообщение
// 661,260, // Volume total   699 --    // 662,418, // Error          701 --
// 662,244, // Error          701 --    //39,73,  // unit for mass flow,kg/s

//4924,3,  //байт порядок
2601,1,   //сч1 массовый расход
2801,2,   //сч2 объемный расход
2605,1,   //сч1 прямой расход Forward flow  only Flow direction code:
2805,1,   //сч2 прямой расход Forward flow  only Flow direction code:
2602,0,  //сч1 gram
2803, 0,  //сч2 cm3

2101,6,  // unit for mass flow,kg/hour
2103,18,  // Volume flow  l/h
2107,4,  // Density unit kg/m3 --
2109,0,  // unit for temperature, C
6813, 0  //блокировка отключена по умолчанию
...
void f_init_MVD(int ii)
{
  ...
  // printf("\n\r init MVD Promass MVD_fn[0]=%02d  MVD_fl[ii]=%02d ", MVD_fn[0], MVD_fl[ii]);// qqq 
  ...
  switch (MVD_fl[ii])
  {
    ...
    case 1:  // проверяет текущее значение порядка
      ...
      if(MVD_fn[ii] != 0) break;
      MVD_fn[ii]=MVD_RD_I;;
      s_MVD[0].r_addr=4924;//4915; 521; // I521  Byte order code for float registers
      MVD_fl[ii]++;
      //MVD_fl[ii]=-1;
    break;
    ...
    case 2:
      ...
      if((MVD_fn[ii] == F_FAULT ) || (s_MVD[ii].i == 1) || (s_MVD[ii].i == 256) ) //  ||   Promass порядок 2-3-0-1 для совместимости с Emerson kod=1
      { 
        // byte order is correct or RFT9739 device connected
        //printf("\n\r byte order is correct ");// qqq        
        //MVD_fl_ord[ii]=3; //порядок 1-0-3-2
        MVD_fl_ord[ii]=0;
        MVD_fl[ii]=3;
        ii_MVD_i=0;
        ii_MVD_fl=0;
      ...
      MVD_fn[ii]=MVD_WR_I;
      s_MVD[ii].r_addr=4924; // I521  Byte order code  for float registers
      s_MVD[ii].i = 1;  //512=2
      if((n_send[ii]++) > 3)
      {
        flag_mvd_ton=0;
    ...
    case 52:
      MVD_fn[ii]=MVD_WR_F;
      s_MVD[ii].r_addr=5138;
      s_MVD[ii].f=cutoff_off_M;
      MVD_fl[ii]=53;
    break;
...
//----------------------------------------------
  ...
  //======AN====================================
  #if(Shtray==2)
    if(count < 36) //Shtray = 2
    {
      goto m_tout_err;
    }
//  lerr_MVD=(long int)f_get_float( &cb_COM[ii][0],1);
    s_MVD[nn].MassT_ml= f_get_float( &cb_COM[ii][0], MVD_fl_ord[0]); // ch1
    s_MVD[nn].MassT_st= f_get_float( &cb_COM[ii][4], MVD_fl_ord[0]);//ch1 ?????
    s_MVD[nn].VolT_ml = f_get_float( &cb_COM[ii][8], MVD_fl_ord[0]);  // ch2
    s_MVD[nn].VolT_st = f_get_float( &cb_COM[ii][12], MVD_fl_ord[0]);  // ch2 ????       
    ftmp =             f_get_float( &cb_COM[ii][16], MVD_fl_ord[0]); //Dens
    if(ftmp <=0) ftmp=0.00000001;
    s_MVD[nn].Dens = ftmp;        
    s_MVD[nn].Temp=    f_get_float( &cb_COM[ii][20], MVD_fl_ord[0]);
    s_MVD[nn].FlowM= f_get_float( &cb_COM[ii][24], MVD_fl_ord[0]); //  КГ/ч
    s_MVD[nn].FlowV =  f_get_float( &cb_COM[ii][28], MVD_fl_ord[0]); // л/ч var[42]=138
    //s_MVD[nn].Compos =  f_get_float( &cb_COM[ii][32], MVD_fl_ord[0]); //?????? Error Integer Message    
    //==========AN=============
    mas_ch1_cur = (s_MVD[0].MassT_st * 10000000) + s_MVD[0].MassT_ml;
    vol_ch2_cur = (s_MVD[0].VolT_st  * 10000000) + s_MVD[0].VolT_ml ;
    s_MVD[0].MassT=(float)((mas_ch1_cur-mas_ch1_start)/1000);//граммы в кг
    s_MVD[0].VolT =(float)((vol_ch2_cur-vol_ch2_start)/1000); 
    s_MVD[0].MassI = (float)(mas_ch1_cur/1000);
    s_MVD[0].VolI  = (float)(vol_ch2_cur/1000);
...
//       break;
      ...
      case mVolI:    // чтение Vol inv-ry
        //08.02.2022 YN
        #if(Shtray==2)
          s_MVD[nn].VolI= ( (s_MVD[0].VolT_st * 10000000) + f_get_float( cb_COM[ii], MVD_fl_ord[0]) )/1000;
        #else
  ...
  switch(fn)
 {
  ...
  case MVD_RD_F: // чтение Float
    ...
    //08.02.2022 YN
    #if(Shtray==2)
      if(cb_COM[ii][1] != 3) if (cb_COM[ii][1] != 4) goto m_crc_err;
      if(cb_COM[ii][2] != 4) goto m_crc_err;
    #else
      if(cb_COM[ii][1] != 3) goto m_crc_err;
      if(cb_COM[ii][2] != 4) goto m_crc_err;
    #endif
...
//08.02.2022 YN
 MVD_Rqst_T[0].timeout=1000; //или  define ToutAnsMVD_rtu 100xxx
 ...
 //08.02.2022 YN
  #if (Shtray==2)  //Promass
    return  f_MVD_WR((int)0,(int) MVD_WR_I,(int)5121,(int)1,(long int) 0,(float)0);
  #else  
...
float ftmp;
  ...
  if( f_timer(tm_mvd_p,t_mvd_cor-1000 )==0 )  //t_mvd_cor KA-p3
  {
   fl_VI=0;
   fl_MI=0;
   cl_MVD_sw=0;
    ...
    case 0:
      ...
      /* if( ( fabs(s_MVD[0].FlowM) > F_Dns_off) && (flag_prok == 0) )
       ...
       // установка значения среза массового расхода для режима ожидания
        MVD_fn[0]=MVD_WR_F;
        s_MVD[0].r_addr=5138; // Cutoff for mass flow
