...
int num_out7=0;  //  подключение дисплея , по F1 ( = 9 IF USING OUT SIGNAL OF MODULE 7188XA)
...
float    F_Dns_off  =200;        //kg/m3
//int counterOfEmptyDensity = 0;
...
//--------------------------
   case  5:
      ...
      #if(Shtray==2) //reg 2609 val=1 сброс + суммировать   
         //для promass не сбрасывать
         // if( f_MVD_WR((int)0,(int)MVD_WR_I,(int) 2609, (int) 1,(long int) 0,(float)0)== 0)
         mas_ch1_start = mas_ch1_cur;
         vol_ch2_start = vol_ch2_cur;
         MVD_t_rslt[0] = 1;
         sw_dlv_liq=6;
      #else
         MVD_t_rslt[0]=0;
...
//-----------------------------
   ...
   case  7:
      ...
      //08.02.2022 YN
      #if(Stray==2) // Имит. измерения 2-объемный расход 1-массовый
         //if( f_MVD_WR((int)0,(int)MVD_WR_I,(int) 6813,(int)2,(long int) 0,(float)0)== 0)
         MVD_t_rslt[0]=1;
         sw_dlv_liq=8;
         break;
      #else
   ...
   case 8:
      ...
      if( MVD_t_rslt[0]>0)
      {
         ...
         //08.02.2022 YN
            ...
            break;
         #else 
            //if( f_MVD_WR((int)0,(int)MVD_WR_F,(int) 5138,(int)0,(long int) 0,cutoff_on_V) == 0)
            MVD_t_rslt[0]=1;
            sw_dlv_liq=9;
            break; 
         #endif
...
//  flag_flch=0;
      ...
      if(dose_dlvr != vBIG_P)
      {
         //02.02.2022 YN
            ...
            {
               if ( Flag_mass != 0 )
                  MmiPrintf("Отп. до опорожнения, кг       ");
               else
                  MmiPrintf("Отп. до опорожнения, л       ");
            }
            else
            {
               if ( Flag_mass != 0 )
                  MmiPrintf("Отп.дозы %g кг         ",dose_dlvr);
               else
                  MmiPrintf("Отп.дозы %g л          ",dose_dlvr);
            }
         #else
            if ( Flag_mass != 0 )
      ...
      if( f_timer(t_prep_MVD, Tim_Prep1 ) == 0 )
       {
         ...
         if((s_MVD[0].Dens > F_Dns_off) || (flag_prok != 0)) ;
         else 
         {
            Flag_dns_flow=1;
            t_prep_MVD1=TimeStamp;
         ...
         { // антидребезг прошел
           ...
           sw_dlv_liq=111;
            ...
            if(flag_prok == 0)
            {
               ...
               //08.02.2022 YN
               #if (Shtray==2) 
                  if( f_MVD_WR((int)0,(int)MVD_WR_F,(int) 5138,(int)0,(long int) 0,(float)0)== 0)
               #else
       ...
       {  // время начальной проверки вышло, есть утечка или нет давления //низкая плотность
         if(Flag_dns_flow != 0)
...
//    MmiGotoxy(0,1);   MmiPuts(list_avt[25]);
    ...
    //23.02.2022 YN
    /*else if(s_MVD[0].Dens < F_Dns_off)
    {
       if(fillingFlag == WHOLE)
       {
          if(counterOfEmptyDensity == 1000)
          {
            counterOfEmptyDensity = 0;
            key__1=-1;
            MmiGotoxy(0,1);   MmiPuts(list_avt[25]);
...
m_esc:
       ...
       result_dlv=1; //  Нажата кнопка   ESC
        ...
        MmiGotoxy(0,15);   MmiPuts(list_avt[28]);  //" ESC - меню                   ",//28
         ...
         //08.02.2022 YN
         #if (Shtray==2) 
            if( f_MVD_WR((int)0,(int)MVD_WR_F,(int) 5138,(int)0,(long int) 0,cutoff_off_M)== 0)
            {
