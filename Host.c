...
//08.02.2022 YN
#if(Shtray == 2)
int MVD_fl_ord[max_MVD]={0,0}; // порядок следования байт для числа float
#else    
...
//  printf(" %02X<<\n\r",cb_COM[ii][6]);
  ...
  //08.02.2022 YN
  /*case RD_DOUBLE: // Shtray=3
    if (cb_COM[ii][1] != 3)
    if (cb_COM[ii][1] != 4)         
    goto m_err;
    if(cb_COM[ii][2] != 8) goto m_err;

    //08.02.2022 YN
    //#if ( (Shtray==0) )
    #if ( (Shtray==0) || (Shtray==2))
      massI_KR=f_get_double( cb_COM[ii] ); //, MVD_fl_ord[0]
    #endif
    printf(" %lf",massI_KR);     
  break;*/
