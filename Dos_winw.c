...
#if defined(GAZAVOZ)
int WHOLE = 1;
float beginValue = 12345.0; // л
float beginMassa = 12345.0; // кг
int fillingFlag = 0;
...
//08.02.2022 YN
  ...
  char str_x_shtray[]="Введите коррекцию объема:";
  char str_x_0[]="Введите коррекцию массы:";
...
void  f_prn_begin()
{
 ...
 m1:
  ...
  //27.10.2021 YN
    ...
    //02.02.2022 YN
    #if defined(GAZAVOZ)
      MmiGotoxy(0,0); MmiPrintf("    ООО Факом технолоджиз     ");
    #else
  ...
  //02.02.2022 YN
  #if defined(GAZAVOZ)
    MmiGotoxy(1,2);   MmiPrintf("    Система учета отпуска     ");
    MmiGotoxy(1,3);   MmiPrintf("     СУГ на автоцистерне      ");
  #else
          ...
          {
            {
                  ...
                  if( waTempOn ) 
                  {
                    ...
                    else
                    {
                      ...
                      else
                      {
                        //02.02.2022 YN
                        #if defined(GAZAVOZ)
                          MmiGotoxy(0,9); MmiPrintf("Температура    %7.2f C        ",s_MVD[0].Temp);
                        #else
...
#endif
              {
                ...
                if(OUT_VAR & OUT2)
                {
                  MmiGotoxy(0,13); MmiPrintf("Состояние клапана:   Открыт  ");
                }
                else
                {
                  MmiGotoxy(0,13); MmiPrintf("Состояние клапана:   Закрыт  ");
                }
...
MmiGotoxy(0,i1++); MmiPrintf("Нет обновл.данных для журнала");
    ...
    if(itmp & DNS_not_off)
    {
      ...
      MmiGotoxy(0,i1++); MmiPrintf(" Низкая плотность для отпуска ");
    if(i1 >= n_mmi_str) return;
    }
...
float ftmp,ftmp1;
 ...
 MmiGotoxy(0,9);   MmiPrintf(" Расход    %10.2f л/ч",s_MVD[0].FlowV);
 MmiGotoxy(0,10);   MmiPrintf(" Плотность %10.2f кг/м3",s_MVD[0].Dens);
 MmiGotoxy(0,11);   MmiPrintf(" Темп.расхдм.  %6.1f град.C",s_MVD[0].Temp);
...
//08.02.2022 YN
  ...
  case 12:
   if(MVD_fn[0] != F_FAULT)
   { MoveToXY(0,8);MmiPrintf("Сдвиг масс. расхода: %.6f",s_MVD[0].f);}
//float sdv_m_flow=1.0, sdv_v_flow=1.0, sdv_dens=1.0;
     sdv_m_flow=s_MVD[0].f;

  case 32:
   MVD_fn[0]=MVD_RD_F;
   s_MVD[0].r_addr=5519; // Коэф. масс. //   s_MVD[0].r_addr=195;  // Cuttoff for mass flow
   sw_mvd_m1=13;
   break;

  case 13:
   if(MVD_fn[0] != F_FAULT)
  { MoveToXY(0,9);MmiPrintf("Коэф. масс. расхода: %.6f",s_MVD[0].f);}
   k_m_flow=s_MVD[0].f;//   k_v_flow=s_MVD[0].f;

   MVD_fn[0]=MVD_RD_F;
   s_MVD[0].r_addr=5525; //Сдвиг объем. расхода
   sw_mvd_m1=14;
   break;

  case 14:
   if(MVD_fn[0] != F_FAULT)
  { MoveToXY(0,10);MmiPrintf("Сдвиг объем.расхода: %.6f",s_MVD[0].f);}
   sdv_v_flow=s_MVD[0].f;   //   k_dens=s_MVD[0].f;
//================================================   
   MVD_fn[0]=MVD_RD_F;
   s_MVD[0].r_addr=5523; // Коэф. объем
   sw_mvd_m1=15;
   break;
  case 15:
   if(MVD_fn[0] != F_FAULT)
  { MoveToXY(0,11);MmiPrintf("Коэф. объем.расхода: %.6f",s_MVD[0].f);}
   k_v_flow=s_MVD[0].f;

   MVD_fn[0]=MVD_RD_F;
   s_MVD[0].r_addr=5529; // Сдвиг плотности
   sw_mvd_m1=16;
   break;   
  case 16:
   if(MVD_fn[0] != F_FAULT)
  { MoveToXY(0,12);MmiPrintf("Сдвиг плотности    : %.6f",s_MVD[0].f);}
   sdv_dens=s_MVD[0].f;
   MVD_fn[0]=MVD_RD_F;
   s_MVD[0].r_addr=5527; // Множитель плотн.
   sw_mvd_m1=17;
   break;   
  case 17:
   if(MVD_fn[0] != F_FAULT)
  { MoveToXY(0,13);MmiPrintf("Коэф. плотности    : %.6f",s_MVD[0].f);}
   k_dens=s_MVD[0].f;
...
// отображает параметры коррекции расхода  MVD  на MMI
  ...
  switch(sw_mvd_m1)
  {
    ...
    //08.02.2022 YN
      ...
      case 31:
        MVD_fn[0]=MVD_RD_F;
        s_MVD[0].r_addr=5138;  //Значение включения    MM=195
        sw_mvd_m1=12;
      break;
      ...
      case 12:
        ...
        { 
          MoveToXY(0,4); 
          MmiPrintf(" Значение включения: %8.1f кг/ч",s_MVD[0].f);
        }
        k_m_flow=s_MVD[0].f;
      ...
      /*case 32:
        MVD_fn[0]=MVD_RD_F;
        s_MVD[0].r_addr=5104;  //Значение выключения 0-100%     MM 197 Cuttoff for vol flow
         sw_mvd_m1=13;
      break;
      ...
      case 13:
        ...
        { 
          MoveToXY(0,5); 
          MmiPrintf("Значение выключения: %8.1f  %",s_MVD[0].f);
        }
        k_v_flow=s_MVD[0].f;*/
    ...
    case 12:
      ...
      { 
        MoveToXY(0,4); 
        MmiPrintf("Срез м. расх.: %8.1f кг/ч",s_MVD[0].f);
      }
      k_m_flow=s_MVD[0].f;
    ...
    case 13:
      ...
      { 
        MoveToXY(0,5); 
        MmiPrintf("Срез об.расх.: %8.1f  л/ч",s_MVD[0].f);
      }
      k_v_flow=s_MVD[0].f;
...
int i,i2;
unsigned long int ltmp,i1;
// чтение прогресса при калибровке

  if((MVD_fn[0] != 0) && (MVD_fn[0] != F_FAULT))
  {
    if( f_timer(tm_mvd,f_MVD_tim ) )
    {
   MoveToXY(0,sw_MMI_cur); MmiPrintf(" !Таймаут чтения MVD Promass");
        ff_serv=NULL;
    }
    return;
  }
  ...
  case 0:
   ...
   MVD_fn[0]=MVD_RD_I;
   s_MVD[0].r_addr=6797; //регистр прогресса
   sw_mvd_m1++;
   break;
...
// FLCH
#if defined(GAZAVOZ)
  "Масса опред.п.BIG",      // 137
  "Масса опред.п.LTL",      // 138
  "Масса нач.опред.п.",       // 139
  "Время переполн,тик",     // 140
  "Время запр.пер.тик",    // 141
#else
  "Ав.кн.Нижн. N вх.",      // 137
...
//          MmiGotoxy(0,2);    MmiPuts("2  Версия ПО ");
            ...
            //02.02.2022 YN
            #if defined(GAZAVOZ)
              MmiGotoxy(0,10);    MmiPuts("5  Ручное открытие клапана  ");
            #endif
          ...
          if(PageD==0)
            {
              ...
              //02.02.2022 YN
                ...
                if( !flagFirstTurnOn )
                {
                  MmiGotoxy(0,7);  MmiPrintf("       Загрузка системы...       ");
                }
                else
                {
                  MmiGotoxy(0,6);  MmiPrintf("Масса инв.  :%9.1f кг",s_MVD[0].MassI);
                  MmiGotoxy(0,8);  MmiPrintf("Объем инв.  :%9.1f л ",s_MVD[0].VolI);
                  MmiGotoxy(0,10); MmiPrintf("Плотность   :%7.1f кг/м3  ",s_MVD[0].Dens);
                  MmiGotoxy(0,12); MmiPrintf("Температура :%7.1f C      ",s_MVD[0].Temp);
                }
              #else
                ...
                //10.06.2021 YN
                  ...
                  MmiGotoxy(0,12);  MmiPrintf("Перепад     :%7.4f МПа    ",Press_Drop);
                #elif (PressureDrop == 2)
                  MmiGotoxy(0,10);  MmiPrintf("Температура :%7.2f C      ",s_MVD[0].TempR);
                  MmiGotoxy(0,11);  MmiPrintf("Давление    :%7.4f МПа    ",s_MVD[0].Press);
                  if(Filter_for_slave) 
 ...
 if((id_ok !=0)&& (mode_hst == 0) )
 {
    ...
    else
    {
          //27.10.2021 YN
            ...
            //02.02.2022 YN
            #if defined(GAZAVOZ)
              MmiGotoxy(0,4);    MmiPuts("1 Задать дозу отпуска СУГ ");
              MmiGotoxy(0,6);    MmiPuts("2 Отпуск до опорожнения   ");
            #else
          ...
          {  
              //02.02.2022 YN
                ...
                if(fillingFlag == WHOLE)
                {
                  ...
                  MmiPrintf("О=%8.2f кг,%8.2f л ",s_MVD[0].MassT,s_MVD[0].VolT);
                  MmiGotoxy(0,10);
                  MmiPuts("4 Продолжить до опорожнения   ");
                }
                else
                {
                  MmiGotoxy(0,2);
                  ...
                  if ( Flag_mass != 0 ) MmiPrintf("Д=%8.2f кг    ",dose_dlvr);
                  else MmiPrintf("Д=%8.2f л    ",dose_dlvr);
                }
              #else
                MmiGotoxy(0,8);
                MmiPuts("4 Продолжить :");
          ...
          else if(dose_dlvr > 0)
          {
              ...
              #if defined(GAZAVOZ)
                MmiGotoxy(0,10);
                if(fillingFlag == WHOLE) MmiPuts("4 Старт до опорожнения"); 
                else MmiPuts("4 Старт ");
              #else
                MmiGotoxy(0,8);
                MmiPuts("4 Старт ");
              #endif
...
//            MmiGotoxy(0,0);    MmiPuts(" Меню 3.Журнал событий.");
            ...
            //02.02.2022 YN
            #if defined(GAZAVOZ)
              MmiGotoxy(0,0);  MmiPrintf("      Журнал событий. ");
...
//   tdat->tm_mday,tdat->tm_mon+1,tdat->tm_year-100,tdat->tm_hour,tdat->tm_min,tdat->tm_sec);
   ...
   tdat->tm_mday,tdat->tm_mon+1,tdat->tm_year-100);
            ...
            //02.02.2022 YN
              ...
              //10.02.2022 YN
              MmiGotoxy(0,12);    MmiPuts("4  Показания счетчика СУГ");
...
m_m3:
  ...
  //MmiGotoxy(0,0);  MmiPrintf("АСН %d. Сервисные функции.",n_ASN);
            ...
            //02.02.2022 YN
            #if defined(GAZAVOZ)
              MmiGotoxy(0,8);    MmiPuts("4  Время, дата");
            #else
        ...
        //02.02.2022 YN
        #if defined(GAZAVOZ) 
          else if(key == '5') // 5 Ручное открытие клапана
          {
...
m_19:
            ...
            OUT_VAR_c=0xffff;
            MVD_fn[0]=MVD_WR_F;
            s_MVD[0].r_addr=5138; // разрешение счета
...
// MmiGotoxy(0,0); MmiPrintf(" Состояния аналоговых входов:");
    //02.02.2022 YN
    #if defined(GAZAVOZ)
      MmiGotoxy(0,0);  MmiPrintf("    Состояния входов:    ");
    #else
...
m_m9:
         ...
         f_clr_scr_MMI();
            //02.02.2022 YN
            #if defined(GAZAVOZ)
              MmiGotoxy(0,0);  MmiPrintf(" Коррекция расхода.");
            #else
...
//            MmiGotoxy(0,2);    MmiPuts("2 Коррекция объемного расхода");
 ...
 #endif 
#if(Shtray==2)
            MmiGotoxy(0,1);    MmiPuts("1 Сдвиг массового расхода");
            MmiGotoxy(0,2);    MmiPuts("2 Коэф. массового расхода");
            MmiGotoxy(0,3);    MmiPuts("3 Сдвиг объемного расхода");            
            MmiGotoxy(0,4);    MmiPuts("4 Коэф. объемного расхода");
            MmiGotoxy(0,5);    MmiPuts("5 Сдвиг плотности");
            MmiGotoxy(0,6);    MmiPuts("6 Коэф. плотности");                        
#endif 
...
m_m9s:
         ...
         f_clr_scr_MMI();
            #if defined(GAZAVOZ)
              MmiGotoxy(0,0);  MmiPrintf("     Срез расхода.         ");
            #else
              MmiGotoxy(0,0);    MmiPrintf("АСН %d. Срез расхода.",n_ASN);
            #endif
            MmiGotoxy(0,1);    MmiPuts("1 Срез массового расхода");

            //17.02.2022 YN MmiGotoxy(0,2);    MmiPuts("2 Срез объемного расхода");
...
//17.02.2022 YN
 ...
 /*========================================*/
  ...
  //02.02.2022 YN
    ...
    case 50:
      ...
      MmiGotoxy(0,0); MmiPrintf("Меню. Ручное открытие клапана ");
      MmiGotoxy(0,2); MmiPrintf("Нажать 1 для откр/закр клапана");
      if(key==ESC)    /* переход в меню */
      {
        MVD_fn[0]=MVD_WR_F;
        s_MVD[0].r_addr=5138; // запрет счета
      ...
      else
      {
        if(OUT_VAR & OUT2)
        {
          MmiGotoxy(0,3); MmiPrintf(" Состояние клапана:   открыт  ");
        }
        else
        {
          MmiGotoxy(0,3); MmiPrintf(" Состояние клапана:   закрыт  ");
        }
      }
      ...
      MmiGotoxy(0,8);   MmiPrintf("Температура    %7.2f C        ",s_MVD[0].Temp);
      MmiGotoxy(0,9);   MmiPrintf("Плотность      %7.2f кг/м3   ",s_MVD[0].Dens);

      MmiGotoxy(0,11);  MmiPrintf("Расход       %8.1f л/ч       ",s_MVD[0].FlowV);

      MmiGotoxy(0,14);    MmiPuts("ESC  Меню");
 ...
 //  Текущее значение.
       if((key==ESC)||(key==ENTER))    /* переход в меню */
       {
         ...
         //08.02.2022 YN
         #if(Shtray==2)
          s_MVD[0].r_addr=5138;  //Значение отсечки
         #else
...
m_m9_1:
          ...
          f_clr_scr_MMI();
          MmiPuts("Введите сдвиг масс. расхода:" );
          k_mv_tmp = sdv_m_flow;
          flag_mass=1;
m_m9_1_1:
...
m_m9_2:
          ...
          SetDisplayPage(EmptPage); // ввод значения
          f_clr_scr_MMI();
          MmiPuts("Введите коэфф.масс.расхода:" );
...
m_m9_3:
          ...
          SetDisplayPage(EmptPage); // ввод значения
          f_clr_scr_MMI();
          MmiPuts("Введите сдвиг объемн.расхода:" );
          k_mv_tmp=sdv_v_flow;
          flag_mass=2;
          goto m_m9_1_1;
       }
...
m_m9_4:
          ...
          SetDisplayPage(EmptPage); // ввод значения
          f_clr_scr_MMI();
          MmiPuts("Введите коэфф.объемн.расхода:" );
          k_mv_tmp=k_v_flow;
          flag_mass=3;
          goto m_m9_1_1;
       }
...
m_m9_5:
          ...
          SetDisplayPage(EmptPage); // ввод значения
          f_clr_scr_MMI();
          MmiPuts("Введите сдвиг плотности:" );
          k_mv_tmp=sdv_dens;
          flag_mass=4;
          goto m_m9_1_1;
       }
...
m_m9_6:
          ...
          SetDisplayPage(EmptPage); // ввод значения
          f_clr_scr_MMI();
          MmiPuts("Введите коэффициент плотности:" );
          k_mv_tmp=k_dens;
          flag_mass=5;
          goto m_m9_1_1;
       }
...
m_m5_4_2:
       ...
       //02.02.2022 YN
        ...
        if(key=='3')
        {
          Flag_mass = 1; //отпуск по массе
          MmiGotoxy(0,11);    MmiPuts("Отпуск по: массе, кг.     ");
          //goto m_m5_4;
        }
        if(key=='4')
        {
          Flag_mass = 0; //отпуск по объему
          MmiGotoxy(0,11);    MmiPuts("Отпуск по: объему, л.     ");
          //goto m_m5_4;
        }
       #endif
      ...
      // введенное значение в k_mv_tmp
          if(flag_mass)
          {
           ...
           chg_new= k_mv_tmp;
            ...
            //02.02.2022 YN 
            #if defined(GAZAVOZ)
              beginMassa = k_mv_tmp; //масса вмещаемая цистерной, кг
            #else
          ...
          else
          {
           ...
           chg_new= k_mv_tmp;
            ...
            //02.02.2022 YN 
            #if defined(GAZAVOZ)
              beginValue = k_mv_tmp; //объем цистерны, л
            #else
...
m_m1:
          ...
          MmiGotoxy(0,3);    MmiPuts("1  На последний расч. час");
          MmiGotoxy(0,5);    MmiPuts("2  За последние 5 суток");
          MmiGotoxy(0,7);    MmiPuts("3  На указанную дату");
          MmiGotoxy(0,9);    MmiPuts("4  На текущий момент");
          MmiGotoxy(0,11);   MmiPuts("5  Значение расчетного часа");
          f_d_ESC();
          sw_mmi=300;
          break;
        }
...
// if( _fmemcmp( &ADC[0], &ADC_c[0], sizeof(ADC_c4)) != 0)
    {
      ...
      //10.06.2021 YN
        ...
        MmiGotoxy(0,4); MmiPrintf(" Перепад      %8g (In%d)",(float)ADC[2],analog_num[2]-1);
      #elif (PressureDrop == 2)
        MmiGotoxy(0,2); MmiPrintf(" Давление     %8g (In%d)",(float)ADC[0],analog_num[0]-1);
        MmiGotoxy(0,3); MmiPrintf(" Температура  %8g (In%d)",(float)ADC[1],analog_num[1]-1);
...
m_m5_1:
        ...
        //02.02.2022 YN
        #if defined(GAZAVOZ)
          MmiGotoxy(0,0);     MmiPrintf("      Отпуск дозы СУГ         ");
          fillingFlag = 0;
        #else
...
m_m5_1o:
        ...
        //02.02.2022 YN
          ...
          if(fillingFlag == WHOLE)
          {
            MmiPrintf("Отпуск до опорожнения, ");
          }
          else
          {
            MmiPrintf("Отп.дозы %g ",dose_dlvr);
          }
        #else
...
m_m5_4:
         ...
         f_clr_scr_MMI();
            ...
            //02.02.2022 YN
            #if defined(GAZAVOZ)
              MmiGotoxy(0,0);    MmiPrintf(" Параметры цистерны ");
              MmiGotoxy(0,3);    MmiPuts("1 Изменить объем цистерны");
              MmiGotoxy(0,4);    MmiPuts("2 Изменить массу вмещ.продукта ");
              MmiGotoxy(0,5);    MmiPuts("3 Отпуск по массе ");
              MmiGotoxy(0,6);    MmiPuts("4 Отпуск по объему ");
              MoveToXY(0,9);    MmiPrintf("Объем цистерны  :%6.0f л",beginValue);
              MoveToXY(0,10);   MmiPrintf("Масса вмещ.прод.:%6ld кг",(long int)beginMassa);
              if(Flag_mass == 1) 
              {
                MmiGotoxy(0,11);    MmiPuts("Отпуск по: массе, кг.     ");
              }
            ...
            MmiGotoxy(0,14);    MmiPuts("ESC  Возврат");
